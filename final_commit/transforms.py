...
@PIPELINES.register_module
class ShrinkBoxes(object):
    def __init__(self, inpaint_radius=3, max_num_boxes=0):
    ...
    def __call__(self, results):
        ...
        indexes = list(range(num_boxes))
        np.random.shuffle(indexes)
        indexes = indexes[0:self.max_num_boxes]

        selected_boxes = boxes[indexes]
        ...
        boxes_copy = np.floor(boxes)
        boxes_copy = boxes_copy.astype(np.int32)
        ...
        img_copy = cv2.inpaint(img_copy, mask, self.inpaint_radius, cv2.INPAINT_NS)

        results['img'] = img_copy
        results['gt_bboxes'] = selected_boxes
        results['gt_labels'] = selected_labels

        # for curr_img, curr_boxes, idx in zip([img, img_copy], [boxes_copy, selected_boxes], [0, 1]):
        #     curr_boxes = curr_boxes.astype(np.int32)
        #     for box in curr_boxes:
        #         cv2.rectangle(curr_img, (box[0], box[1]), (box[2], box[3]), (255, 9, 9), thickness=5)
        #     cv2.imwrite('{}.jpg'.format(idx), curr_img)

        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += 'max_num_boxes={}'.format(self.max_num_boxes)
        return repr_str
